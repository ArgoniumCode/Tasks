...
public class ForkOut {
	public static void main(String[] args) throws IOException {
		...
		var forks = findForks(tree);

		Collections.sort(forks);

		for (var fork : forks) {
			writer.write(String.valueOf(fork));
			writer.newLine();
		}
	...
	private static List<Integer> findForks(TreeNode root) {
		var forks = new ArrayList<Integer>();
		collectForks(root, forks);

		return forks;
	}

	private static void collectForks(TreeNode node, List<Integer> forks) {
		if (node == null) return;

		if (node.left != null && node.right != null) {
			forks.add(node.value);
		}
